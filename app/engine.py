...
class RealV2Engine:
    ...
    def update_data(self):
        ...
        # Save back (minimal update)
        # We would ideally save it but per instructions 'app' folder might move, so we use it in memory
        ...
        now = datetime.now()
        if now.day == 1:
            print("1st of the month: Triggering Beta recalculation...")
            try:
                # Find the most recent date that has 60 days of future data
                # Typically about 3-4 months ago
                search_date = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
                
                # Load existing results to see what's missing
                ...
                for current_check in missing_dates:
                    ...
                    if res_beta is not None:
                        ...
                        # Load again to ensure we have latest state
                        existing_beta = pd.read_excel(self.beta_file_path, sheet_name='P200_F60')
                        existing_beta['Date'] = pd.to_datetime(existing_beta['Date'], errors='coerce')
                        existing_beta = existing_beta.dropna(subset=['Date'])
                        
                        # Update the 'Actual' value for this date
                        existing_beta.loc[existing_beta['Date'] == current_check, 'Actual'] = res_beta
                        
                        # Recalculate 10-month average for all rows
                        # We use rolling mean on the Actual column, then forward-fill
                        # This ensures that if a month (like Jan 1st) is NaN, 
                        # it inherits the average from the last valid month (Dec 1st).
                        ...
                        # If a month cannot be calculated, probably subsequent won't either
                        break 
