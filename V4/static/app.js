document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('universeUploadBtn').addEventListener('click', () => document.getElementById('universeFileInput').click());
    document.getElementById('universeFileInput').addEventListener('change', handleUniverseUpload);
...
async function checkUniverseStatus() {
    try {
        const res = await fetch('/api/universe/status');
        const data = await res.json();
        const alertDiv = document.getElementById('universeAlert');
        const alertMsg = document.getElementById('universeAlertMsg');

        if (data.status === 'missing_row' || data.status === 'missing_data') {
            alertDiv.style.display = 'flex';
            const reason = data.status === 'missing_row' ? 'New period detected' : 'Stock data missing';
            alertMsg.innerHTML = `<strong>Attention Required:</strong> ${reason} for <b>${data.date}</b>. Please upload the KOSPI 50 Excel file.`;
        } else {
            alertDiv.style.display = 'none';
        }
    } catch (e) {
        console.error('Error checking universe status', e);
    }
}

async function handleUniverseUpload(event) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.message) {
            alert('Universe updated successfully! Refreshing dashboard...');
            location.reload();
        } else {
